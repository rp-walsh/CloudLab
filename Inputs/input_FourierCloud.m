...
Pm = @(x,z,t) Am*cos(alpham*z).*sin(k*x-omega*t);
Pp = @(x,z,t) Ap*cos(alphap*(z-H)).*sin(k*x-omega*t); 
PDist =@(x,z,t) Pm(x,z,t).*((Gmm(x,t)-z)>=0) + Pp(x,z,t).*((z-Gmm(x,t))>0);
PBkg = @(x,z,t) g*(c1m*bb+c2m*gg)*(z-h).^2./2.*((Gmm(x,t)-z)>=0) + g*(c1p*bb+c2p*gg)*(z-h).^2./2.*((z-Gmm(x,t))>0);
%PBkg = @(x,z,rl) g*(c1m*bb + c2m*gg)*(z-h).^2./2.*(rl<0) + g*(c1p*bb + c2p*gg)*(z-h).^2./2.*(rl>=0);
PExact = @(x,z,t) PBkg(x,z,t) + PDist(x,z,t); 
...
Pmz = @(x,z,t) - alpham*Am*sin(alpham*z).*sin(k*x-omega*t); 
Ppz = @(x,z,t) - alphap*Ap*sin(alphap*(z-H)).*sin(k*x-omega*t); 
PzDist = @(x,z,t) Pmz(x,z,t).*((Gmm(x,t)-z)>=0) + Ppz(x,z,t).*((z-Gmm(x,t))>0);
PzBkg = @(x,z,t) g*(c1p*bb + c2p*gg)*(z-h) ;
PzExact = @(x,z,t) PzBkg(x,z,t) + PzDist(x,z,t); 
...
SDist = @(x,z,t) Sm(x,z,t).*((Gmm(x,t)-z)>=0) + Sp(x,z,t).*((z-Gmm(x,t))>0);
sExact = @(x,z,t) sBkg(x,z,t) + SDist(x,z,t); 
...
% Density
rhoDist = @(x,z,t) (-c1m*Sm(x,z,t)-c2m*rTm(x,z,t)).*((Gmm(x,t)-z)>=0) +...
                   (-c1p*Sp(x,z,t)-c2p*rTp(x,z,t)).*((z-Gmm(x,t))>0);
rhoBkg = @(x,z,t) (-c1m*sBkg(x,z)-c2m*rTBkg(x,z)).*((Gmm(x,t)-z)>=0) +...
                   (-c1p*sBkg(x,z)-c2p*rTBkg(x,z)).*((z-Gmm(x,t))>0);
rhoExact = @(x,z,t) rhoBkg(x,z,t) + rhoDist(x,z,t);
...
wDist = @(x,z,t) Wm(x,z,t).*((Gmm(x,t)-z)>=0) + Wp(x,z,t).*((z-Gmm(x,t))>0);
wBkg = @(x,z,t) zeros(size(x));
wExact = @(x,z,t) wBkg(x,z,t) + wDist(x,z,t);
wInit = @(x,z) wExact(x,z,0);

% z-velocity
Um = @(x,z,t) k/omega*Am*cos(alpham*z).*sin(k*x-omega*t); 
Up = @(x,z,t) k/omega*Ap*cos(alphap*(z-H)).*sin(k*x-omega*t); 
uDist =@(x,z,t) Um(x,z,t).*((Gmm(x,t)-z)>=0) + Up(x,z,t).*((z-Gmm(x,t))>0);
uBkg = @(x,z,t) zeros(size(x));
uExact = @(x,z,t) uBkg(x,z,t) + uDist(x,z,t);
